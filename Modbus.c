    ...
    if (numberHandlers < MAX_M_HANDLERS)
    {
        ...
        else if (modH->uModbusType == MB_MASTER)
        {
            ...
            modH->xTimerTimeout = xTimerCreate(
                "xTimerTimeout",              // name
                modH->u16timeOut,            // period in ticks
                pdFALSE,                     // one-shot
                (void *)modH->xTimerTimeout, // id
...
/* =======================================================================
 * 🟥 MASTER SECTION
 * Master task, query enqueue helpers, SendQuery, and response processing.
 * Static functions are placed at the top of the section.
 * ======================================================================= */

/* --- Master static prototypes (local to master section) --- */
...
/* --- Build and send a master query --- */
static int8_t SendQuery(modbusHandler_t *modH, modbus_t telegram)
{
    uint8_t u8regsno, u8bytesno;
    uint8_t error = 0;

    xSemaphoreTake(modH->ModBusSphrHandle, portMAX_DELAY); // lock data
...
void StartTaskModbusMaster(void *argument)
{
    ...
    for (;;)
    {
        ...
        xSemaphoreTake(modH->ModBusSphrHandle, portMAX_DELAY); // lock before processing
        ...
        {
            xSemaphoreGive(modH->ModBusSphrHandle); // Release
...
/* --- Process master answers: FC1 & FC3 --- */
static void get_FC1(modbusHandler_t *modH)
{
    uint8_t u8byte = 3;
